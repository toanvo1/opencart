...
public function momo() {
	...
	if (!isset($this->session->data['api_id'])) {
		...
		if (empty($this->session->data['payment_methods'])) {
			...
			// Check if MoMo payment method is available
			$momo_available = false;
			foreach ($this->session->data['payment_methods'] as $payment_method) {
				if ($payment_method['code'] == 'momo') {
					$momo_available = true;
					break;
				}
			}
			if (!$momo_available) {
				$json['error'] = $this->language->get('error_no_momo_payment');
			} else {
				// Set MoMo payment method as the selected method
				foreach ($this->session->data['payment_methods'] as $payment_method) {
					if ($payment_method['code'] == 'momo') {
